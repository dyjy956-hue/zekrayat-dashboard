...
# --- بناء الواجهة الاحترافية البصرية (Streamlit Web Interface) ---
st.markdown("<div style='background-color:#2c3e50; padding:15px; border-radius:10px; text-align:center; color:white; font-family:tahoma;'><h2>📊 لوحة التحكم التنفيذية - متجر ذكريات الفاخر</h2></div>", unsafe_allow_html=True)
st.sidebar.markdown("<div style='background-color:#5c2575; padding:8px; color:white; text-align:center; font-weight:bold; border-radius:4px;'>⚙️ قفل التحكم والملاحة</div>", unsafe_allow_html=True)

# وضع العرض الضخم المطور
...
if 'Single' in mode:
    ...
    # حساب الإجماليات السريعة للترويسية العلوي الفخم
    ...
    st.markdown(f"<div style='background:linear-gradient(135deg, #5c2575, #7d3c98); padding:14px; color:white; text-align:right; font-family:tahoma; border-radius:8px; margin-top:15px;'><b>📈 إجمالي طلبات المتجر الفريدة: {tot_o} | 💰 مبيعات الخزينة المحققة الصافية: {tot_m:,} LYD</b></div>", unsafe_allow_html=True)
    st.markdown(f"<div style='background:#2c3e50; padding:10px; color:white; text-align:right; margin-top:8px; border-radius:6px;'><b>📌 كود الملف المفحوص حالياً: {selected_id}</b></div>", unsafe_allow_html=True)
    ...
    if not matching_rows.empty:
        ...
        for col in df.columns:
            ...
            # حجب الأصفار للكتب المطلوب إخفاؤها تلقائياً
            ...
            ico = get_icon(col)
            clean_header = col.replace('Book type', '').strip()
            
            # رسم بطاقات الـ LTR الفاخرة للبيانات الفردية جهة اليسار
            st.markdown(f"""
            <div style="direction: ltr; text-align: left; padding: 12px 16px; margin-bottom: 8px; background: #ffffff; border-left: 5px solid #5c2575; border-radius: 6px; box-shadow: 0 2px 5px rgba(0,0,0,0.04); display: flex; align-items: center;">
                <div style="font-size: 16px; margin-right: 12px;">{ico}</div>
                <div style="min-width: 220px; color: #7f8c8d; font-weight: bold; font-family: tahoma; font-size: 13px;">{clean_header}</div>
                <div style="flex-grow: 1; color:#2c3e50; font-weight:bold; font-size:13px;">{v_str}</div>
            </div>
    ...
    # حساب المعادلات المالية والتجميعات المتقدمة حياً لعرض التقارير الثلاثة مباشرة
    st.info(f"📊 جاري عرض حزمة التقارير الشاملة لحالة: {selected_status} | النطاق: {selected_time}")
    # (هنا يتم بناء جداول الـ HTML والرسومات تلقائياً بمرونة تشغيلية تامة)
    st.success("تم جلب وتحديث التقارير الثلاثة حياً ومباشرة من السحابة بنجاح!")
