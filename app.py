...
try:
    ...
    # استخدام openpyxl لضمان التوافقية العالية في بيئات الاستضافة السحابية
    df = pd.read_excel(live_url, sheet_name=0, engine='openpyxl') 
    ...
    # بيانات افتراضية لحماية الواجهة من الانهيار التام في حال انقطاع السيرفر السحابي
...
# تنسيق الواجهة عبر CSS لضمان دعم اللغة العربية والاتجاه من اليمين لليسار
st.markdown("""
    <style>
...
# زر التحديث الحي المباشر من السحاب لتفريغ الكاش وفهرسة البيانات من جديد
...
if 'Single' in mode:
    ...
    if not df.empty and 'clean_all' in locals():
        ...
        st.info("الرجاء التحقق من اتصال رابط قوقل شيت وجاهزية البيانات حياً.")
    ...
    if not df.empty and 'clean_all' in locals():
        ...
        # ضبط المنطقة الزمنية يدوياً لضمان توافق التواريخ
        ...
        for idx, row in target_orders.iterrows():
            ...
            table_rows += f"<tr>" \
                          ...
                          f"<td style='padding:10px; border-bottom:1px solid #eee; text-align:center;'><span style='background:#e8f8f8; color:#117a65; padding:4px 8px; border-radius:4px; font-weight:bold;'>{price_display}</span></td>" \
                          f"</tr>"
