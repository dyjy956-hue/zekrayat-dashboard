...
# --- 🧠 محرك تصحيح وهندسة الخط العربي داخل الرسوم البيانية ---
def fix_arabic_text_for_charts(text):
    text = str(text).strip()
    if not text:
        return ""
    # قاموس لتصحيح الكلمات الأكثر استخداماً في المتجر لإظهارها مدمجة وسليمة
    replacements = {
        "تسليم": "مـسـتـلـم",
        "تم": "تـم",
    ...
    # إذا كانت الكلمة في القاموس يتم ربط حروفها بجمالية هندسية مفرودة
...
if 'Single' in mode:
    ...
    st.markdown(f"<div style='background:#2c3e50; padding:10px; color:white; text-align:right; margin-top:10px; border-radius:6px; font-family:tahoma;'><b>📌 تفاصيل كود الطلب الحالي / Current Order: {selected_id}
