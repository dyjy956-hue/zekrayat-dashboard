...
st.set_page_config(page_title="متجر ذكريات", layout="wide")
...
st.markdown("<div style='background:#2c3e50; padding:12px; border-radius:8px; text-align:center; color:white; margin-bottom:15px;'><h2>📊 لوحة تحكم متجر ذكريات الفاخر</h2></div>", unsafe_allow_html=True)
...
if 'Single' in mode:
    ...
    st.markdown(f"<div style='background:#5c2575; padding:8px; color:white; border-radius:4px; margin-bottom:10px;'><b>📌 تفاصيل الطلب: {selected_id}</b></div>", unsafe_allow_html=True)
    ...
    if 'p_date' in clean_all.columns and not tbl.empty:
        if 'اليوم' in selected_time: tbl = tbl[tbl['p_date'] == today]
        elif 'الأسبوع' in selected_time: tbl = tbl[tbl['p_date'] >= (today - datetime.timedelta(days=7))]

    # العدادات العلوية الفاخرة
    u_all = clean_all.drop_duplicates(subset=[id_c])
    c_del = len(u_all[u_all[st_c].str.contains('تسليم|تم|مستلم', na=False)])
    c_shp = len(u_all[u_all[st_c].str.contains('شحن|طريق|مندوب', na=False)])
    c_prp = len(u_all[u_all[st_c].str.contains('تجهيز|تحضير|ورشة', na=False)])
    
    cb1, cb2, cb3 = st.columns(3)
    cb1.markdown(f"<div style='background:linear-gradient(135deg, #2ecc71, #27ae60); padding:12px; border-radius:6px; text-align:center; color:white;'><h3>{c_del}</h3><b>📦 مستلمة</b></div>", unsafe_allow_html=True)
    cb2.markdown(f"<div style='background:linear-gradient(135deg, #e67e22, #d35400); padding:12px; border-radius:6px; text-align:center; color:white;'><h3>{c_shp}</h3><b>🚚 في الشحن</b></div>", unsafe_allow_html=True)
    cb3.markdown(f"<div style='background:linear-gradient(135deg, #3498db, #2980b9); padding:12px; border-radius:6px; text-align:center; color:white;'><h3>{c_prp}</h3><b>🛠️ قيد التجهيز</b></div>", unsafe_allow_html=True)
